...
def task_list(request):
    ...
    calendar_events = CalendarEvent.objects.filter(
        start__lt=query_end, end__gt=query_start, is_active=True, is_hidden=False
    ).order_by("start")
...
def list_tags(_request):
    """List all task tags."""
    tags = TaskTag.objects.all()
    return JsonResponse({"success": True, "tags": [{"id": t.id, "name": t.name} for t in tags]})
...
def create_tag(request):
    ...
    try:
        ...
        return JsonResponse({"success": True, "tag": {"id": tag.id, "name": tag.name}, "created": created})
    except json.JSONDecodeError:
        return JsonResponse({"success": False, "error": _INVALID_JSON}, status=400)
...
def add_tag_to_task(request, task_id):
    ...
    try:
        ...
        return JsonResponse({"success": True, "task_tags": [{"id": t.id, "name": t.name} for t in task.tags.all()]})
    except Task.DoesNotExist:
        return JsonResponse({"success": False, "error": _TASK_NOT_FOUND}, status=404)
...
def create_time_block(request):
    ...
    try:
        ...
        time_block = TimeBlock.objects.create(name=name, start_time=start_dt, end_time=end_dt)
...
def create_task_schedule(request, task_id):
    ...
    try:
        ...
        schedule = TaskSchedule.objects.create(task=task, start_time=start_dt, end_time=end_dt)
...
def create_template(request):
    ...
    try:
        ...
        template = TaskDetailTemplate.objects.create(name=name, content=content, is_default=is_default, order=max_order)
...
def create_view(request):
    ...
    try:
        ...
        view = TaskView.objects.create(name=name, settings=settings, is_default=is_default, order=max_order)
...
def _get_or_create_abandoned_state():
    ...
    abandoned_state, _ = TaskState.objects.get_or_create(
        name="Abandoned", defaults={"order": 999, "bootstrap_icon": "bi-archive", "is_system": True}
    )
...
def _build_abandon_queryset(abandoned_days, abandoned_state, terminal_state, excluded_tag_ids, excluded_state_ids):
    """Build the queryset of tasks eligible for abandonment."""
    threshold_date = timezone.now() - timedelta(days=abandoned_days)

    qs = (
        Task.objects.filter(updated_at__lt=threshold_date)
        .exclude(state=terminal_state)
        .exclude(state=abandoned_state)
...
def process_abandoned_tasks(request):
    ...
    try:
        ...
        terminal_state = TaskState.objects.filter(is_system=False).order_by("-order").first()
        ...
        tasks_to_abandon = _build_abandon_queryset(
            abandoned_days, abandoned_state, terminal_state, excluded_tag_ids, excluded_state_ids
        )
        ...
        if count > 0:
            tasks_to_abandon.update(state=abandoned_state, state_changed_at=timezone.now())
