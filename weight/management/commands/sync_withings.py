...
class Command(BaseSyncCommand):
    ...
    def sync(self, days, sync_all=False):
        ...
        try:
            ...
            measurements_data = client.get_all_weight_measurements(start_date=start_date, end_date=end_date)
    ...
    def _process_measurement(self, measurement_group: dict) -> str:
        ...
        if not measurement_timestamp:
            self.stdout.write(self.style.WARNING(f"Skipping measurement {group_id} - no timestamp"))
            return "skipped"
        ...
        if not weight_measure:
            self.stdout.write(self.style.WARNING(f"Skipping measurement {group_id} - no weight data"))
            return "skipped"
        ...
        if value is None or unit is None:
            self.stdout.write(self.style.WARNING(f"Skipping measurement {group_id} - invalid weight value"))
            return "skipped"
        ...
        weighin, created = WeighIn.objects.update_or_create(
            source="Withings", source_id=str(group_id), defaults=weighin_defaults
        )
        ...
        if created:
            ...
            self.stdout.write(f'  Updated measurement: {weighin.measurement_time.strftime("%Y-%m-%d %H:%M")}')
            return "updated"
