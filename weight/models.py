...
class WeighIn(models.Model):
    ...
    source = models.CharField(
        max_length=50, help_text="Source of the weight data (e.g., 'Whoop', 'Manual', 'Withings')"
    )
    source_id = models.CharField(max_length=255, help_text="ID from the external source system", db_index=True)
    measurement_time = models.DateTimeField(help_text="When the weight measurement was taken")
    weight = models.DecimalField(max_digits=5, decimal_places=2, help_text="Weight in pounds (lbs)")
