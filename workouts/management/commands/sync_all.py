...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--whoop-only", action="store_true", help="Only sync Whoop data (skip other sources)")
    ...
    def handle(self, *_args, **options):
        ...
        self.sync_results["whoop"] = self._run_sync_command("workouts.management.commands.sync_whoop", days)

        if not whoop_only:
            # Sync Withings weight measurements
            self.stdout.write(self.style.HTTP_INFO("\n[2/3] Syncing Withings weight measurements..."))
            self.sync_results["withings"] = self._run_sync_command("weight.management.commands.sync_withings", days)
        ...
        for source, result in self.sync_results.items():
            if result.success:
                self.stdout.write(self.style.SUCCESS(f"✓ {source.upper()}: {result.summary}"))
            else:
                self.stdout.write(self.style.ERROR(f"✗ {source.upper()}: {result.summary}"))
