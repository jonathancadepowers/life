...
class Command(BaseSyncCommand):
    ...
    def sync(self, days, sync_all=False):
        ...
        try:
            ...
            workouts_data = client.get_all_workouts(start_date=start_date, end_date=end_date)
    ...
    def _process_workout(self, workout_data: dict) -> str:
        ...
        if not score:
            self.stdout.write(self.style.WARNING(f"Skipping workout {workout_id} - not yet scored"))
            return "skipped"

        try:
            start_time = datetime.fromisoformat(workout_data["start"].replace("Z", "+00:00"))
            end_time = datetime.fromisoformat(workout_data["end"].replace("Z", "+00:00"))
        except (KeyError, ValueError) as e:
            self.stdout.write(self.style.WARNING(f"Skipping workout {workout_id} - invalid timestamps: {e}"))
            return "skipped"
        ...
        workout, created = Workout.objects.update_or_create(
            source="Whoop", source_id=workout_id, defaults=workout_defaults
        )

        if created:
            self.stdout.write(
                self.style.SUCCESS(
                    f'✓ Created workout: {workout.start.strftime("%Y-%m-%d %H:%M")} ' f"(Sport ID: {workout.sport_id})"
                )
            )
            return "created"
        else:
            self.stdout.write(f'  Updated workout: {workout.start.strftime("%Y-%m-%d %H:%M")}')
            return "updated"
