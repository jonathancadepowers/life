...
def log_nutrition(request):
    ...
    try:
        ...
        try:
            ...
            return JsonResponse({"success": False, "message": "Nutrition values must be valid numbers"}, status=400)

        # Validate non-negative values
        if any(value < 0 for value in [calories, fat, carbs, protein]):
            return JsonResponse({"success": False, "message": "Nutrition values cannot be negative"}, status=400)
        ...
        return JsonResponse({"success": False, "message": f"Error logging nutrition: {str(e)}"}, status=500)
