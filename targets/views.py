...
def get_goals_for_project(request):
    ...
    if show_all:
        ...
        goal_ids = TimeLog.objects.filter(project_id=project_id).values_list("goals__goal_id", flat=True).distinct()
    ...
    try:
        ...
        for project_data in toggl_projects:
            ...
            if project_id and project_name:
                ...
                Project.objects.update_or_create(project_id=project_id, defaults={"display_string": project_name})
                projects_synced += 1
        ...
        for tag_data in toggl_tags:
            ...
            if tag_id and tag_name:
                ...
                Goal.objects.update_or_create(
                    goal_id=str(tag_id), defaults={"display_string": tag_name}  # Use Toggl tag ID as primary key
                )
                goals_synced += 1
        ...
        return JsonResponse({"success": False, "message": f"Error syncing from Toggl: {str(e)}"}, status=500)
...
def save_agenda(request):
    ...
    try:
        ...
        return JsonResponse({"success": True, "message": "Today's agenda has been set!", "day_score": agenda.day_score})

    except Exception as e:
        return JsonResponse({"success": False, "message": f"Error saving agenda: {str(e)}"}, status=500)
...
def _query_toggl_historical(project_id, goal_id, target_date, timezone_offset, request):
    ...
    time_logs = TimeLog.objects.filter(project_id=int(project_id), start__gte=day_start_utc, start__lt=day_end_utc)
    if goal_id:
...
def get_toggl_time_today(request):
    ...
    try:
        ...
        if is_today:
            ...
            total_seconds, debug_info = _query_toggl_historical(
                project_id, goal_id, target_date, timezone_offset, request
            )
...
def get_available_agenda_dates(_request):
    ...
    try:
        ...
        return JsonResponse({"success": True, "dates": date_strings})
    except Exception as e:
...
def get_agenda_for_date(request):
    ...
    try:
        ...
        try:
            ...
            return JsonResponse({"success": True, "agenda": agenda_data})
...
def save_target_score(request):
    ...
    try:
        ...
        if target_num not in (1, 2, 3):
            return JsonResponse(
                {"success": False, "error": f"Invalid target number: {target_num}. Only targets 1-3 can be scored."},
                status=400,
            )
        ...
        return JsonResponse(
            {"success": True, "message": f"Score saved for target {target_num}", "day_score": agenda.day_score}
        )
...
def _get_fasting_stats(start_datetime, end_datetime, days_in_range):
    ...
    fasting_sessions = FastingSession.objects.filter(fast_end_date__gte=start_datetime, fast_end_date__lte=end_datetime)
    days_with_fasts = (
        fasting_sessions.annotate(fast_date=TruncDate("fast_end_date")).values("fast_date").distinct().count()
    )
...
def _get_nutrition_stats(start_datetime, end_datetime, days_in_range):
    ...
    nutrition_entries = NutritionEntry.objects.filter(
        consumption_date__gte=start_datetime, consumption_date__lte=end_datetime
    )
...
def _get_weight_stats(start_datetime, end_datetime):
    ...
    weigh_ins = WeighIn.objects.filter(
        measurement_time__gte=start_datetime, measurement_time__lte=end_datetime
...
def _get_workouts_by_sport(start_datetime, end_datetime, sport_names_dict):
    ...
    workouts = Workout.objects.filter(start__gte=start_datetime, start__lte=end_datetime)
    ...
    return dict(sorted(workouts_by_sport.items(), key=lambda x: x[1]["count"], reverse=True))
...
def _apply_time_percentages(time_by_project, total_time_hours):
    ...
    for project_data in time_by_project.values():
        project_data["percentage"] = (
            round((project_data["total_hours"] / total_time_hours) * 100) if total_time_hours > 0 else 0
        )
        project_data["goals"] = {
            name: {
                "hours": hours,
...
def _get_time_by_project(start_datetime, end_datetime):
    ...
    time_logs = TimeLog.objects.filter(start__gte=start_datetime, start__lte=end_datetime).prefetch_related("goals")

    time_by_project = _accumulate_time_by_project(time_logs)

    # Sort projects and goals by hours descending
    time_by_project = dict(sorted(time_by_project.items(), key=lambda x: x[1]["total_hours"], reverse=True))
    for project_name in time_by_project:
        time_by_project[project_name]["goals"] = dict(
            sorted(time_by_project[project_name]["goals"].items(), key=lambda x: x[1], reverse=True)
        )
...
def _get_monthly_objectives_context(request, end_date, start_date):
    ...
    monthly_objectives = MonthlyObjective.objects.filter(
        start=target_month_first_day, end=target_month_last_day
    ).order_by("label")
...
def _get_todays_activity(request, sport_names_dict):
    ...
    todays_time_logs = (
        TimeLog.objects.filter(start__gte=today_start, start__lte=today_end)
    ...
    return {
        ...
        "fasts": FastingSession.objects.filter(fast_end_date__gte=today_start, fast_end_date__lte=today_end).order_by(
            "-fast_end_date"
        ),
        "nutrition": NutritionEntry.objects.filter(
            consumption_date__gte=today_start, consumption_date__lte=today_end
        ).order_by("-consumption_date"),
        "weighins": WeighIn.objects.filter(measurement_time__gte=today_start, measurement_time__lte=today_end).order_by(
            "-measurement_time"
        ),
...
def _build_objective_id_query(sql, table_name, date_col_sql, objective):
    ...
    if where_match:
        ...
        return (
            f"SELECT {pk_column}, {date_col_sql} FROM {table_name} "
            f"WHERE {where_clause} ORDER BY {date_col_sql} DESC LIMIT 50"
        )
    ...
    if table_name == "targets_dailyagenda":
        ...
        date_range_filter = (
            f"{date_col_sql}::date >= '{objective.start}' " f"AND {date_col_sql}::date <= '{objective.end}'"
        )
    return (
        f"SELECT {pk_column}, {date_col_sql} FROM {table_name} "
        f"WHERE {date_range_filter} ORDER BY {date_col_sql} DESC LIMIT 50"
    )
...
def _format_entry_default(table_name, row_dict, user_timezone):
    ...
    formatters = {
        "fasting_session": lambda r: (
            f"{_fmt(r.get('fast_end_date', ''), user_timezone)}: " f"{r.get('duration_hours', 0):.1f} hour fast"
        ),
        "nutrition_entry": lambda r: (
            f"{_fmt(r.get('consumption_date', ''), user_timezone)}: " f"{r.get('food_name', 'Food entry')}"
        ),
        "weight_weighin": lambda r: (
            f"{_fmt(r.get('measurement_time', ''), user_timezone)}: " f"{r.get('weight_kg', 0):.1f} kg"
        ),
        "workouts_workout": lambda r: (
            f"{_fmt(r.get('start', ''), user_timezone)}: "
            f"Sport {r.get('sport_id', 'Workout')} ({r.get('duration_minutes', 0)} min)"
        ),
        "time_logs_timelog": lambda r: (
            f"{_fmt(r.get('start', ''), user_timezone)}: "
            f"Project {r.get('project_id', 'Unknown')} "
            f"({r.get('duration_minutes', 0) / 60:.1f} hours)"
        ),
...
def get_objective_entries(request):
    ...
    try:
        ...
        if not from_match:
            return JsonResponse({"error": "Could not identify data source table.", "objective_label": objective.label})
...
def get_objective_available_fields(request):
    ...
    try:
        ...
        if not from_match:
            return JsonResponse({"error": "Could not identify table from SQL definition", "fields": []})
        ...
        return JsonResponse({"success": True, "table_name": table_name, "fields": columns})

    except Exception as e:
        return JsonResponse({"error": f"Error fetching fields: {str(e)}", "fields": []}, status=500)
    ...
    try:
        ...
        if MonthlyObjective.objects.filter(objective_id=objective_id).exists():
            return JsonResponse(
                {
                    "error": (
                        f"An objective with this label already exists for "
                        f'{month_abbrev.capitalize()} {parsed["year"]}. '
...
def delete_objective(request):
    ...
    try:
        ...
        if not objective_id:
            return JsonResponse({"error": "Objective ID is required"}, status=400)
        ...
        return JsonResponse({"success": True, "message": f'Objective "{objective_label}" deleted successfully!'})

    except json.JSONDecodeError:
        return JsonResponse({"error": _INVALID_JSON}, status=400)
    except Exception as e:
        return JsonResponse({"error": f"Error deleting objective: {str(e)}"}, status=500)
        ...
        return JsonResponse(
            {
                ...
                "message": f"Refreshed {updated_count} objective(s)"
                + (f", {error_count} error(s)" if error_count > 0 else ""),
            }
        )
