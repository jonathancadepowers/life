...
class Command(BaseCommand):
    ...
    def _migrate_provider(self, provider_config):
        ...
        if not client_id or not client_secret:
            self.stdout.write(
                self.style.WARNING("  \u2298 Skipped: Missing client_id or client_secret in environment variables")
            )
            return False
        ...
        if access_token and refresh_token:
            ...
            self.stdout.write(
                self.style.WARNING(f"    \u26a0 No tokens found - you'll need to run {provider_name}_auth")
            )
    ...
    def handle(self, *_args, **_options):
        ...
        self.stdout.write("")
        self.stdout.write(self.style.SUCCESS("\u2713 OAuth credentials are now stored in the database!"))
        self.stdout.write(self.style.SUCCESS("  Tokens will automatically persist when refreshed."))
        self.stdout.write("")
