...
class FastingSession(models.Model):
    ...
    source = models.CharField(max_length=50, help_text="Source of the fasting data (e.g., 'Zero', 'Manual')")
    source_id = models.CharField(max_length=255, help_text="ID from the external source system", db_index=True)
    duration = models.DecimalField(max_digits=5, decimal_places=2, help_text="Fasting duration in hours")
    fast_end_date = models.DateTimeField(help_text="When the fast ended (UTC)")
