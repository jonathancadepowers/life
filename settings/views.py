...
def add_inspiration(request):
    ...
    if Inspiration.objects.filter(title__iexact=title).exists():
        messages.error(
            request, f'An inspiration with the title "{title}" already exists. Please use a different title.'
        )
        return redirect(reverse("life_tracker_settings") + _ANCHOR_INSPIRATIONS)
    ...
    try:
        ...
        Inspiration.objects.create(image=resized_image, title=title, flip_text=flip_text, type=type_value, url=url)
...
def add_writing_image(request):
    ...
    if request.method == "POST":
        ...
        if image and excerpt:
            WritingPageImage.objects.create(image=image, excerpt=excerpt, enabled=True)
...
def import_outlook_calendar(request):
    ...
    try:
        ...
        messages.success(request, f"Import complete: {created_count} events created, {updated_count} events updated.")
...
def toggle_abandon_day(request, column_name):
    ...
    try:
        ...
        return JsonResponse({"success": True, "is_abandoned": is_abandoned, "date": date_str})
